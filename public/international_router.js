...
var int_router = express.Router(); //Initialisiere Router für Express
...
/**
 * @description Internationale Konvertierung von .docx in .html durch 'mammoth'. Wird bei der Dateiauswahl aufgerufen
 * @name post/convert_docx_to_html_int
 * @param req.file.originalname Liefert den Dateinamen der zu konvertierenden .docx-Datei
 * @param fileInt In Html mit ID ausgezeichnet im internationalen Artikelupload
 */
int_router.post('/convert_docx_to_html_int', upload.single('fileInt'), function(req, res) {
...
/**
 * @description Internationaler Artikelupload, Upload JSON-Datensatz zur REST-API von TYPO3
 * @name post/upload_typo_german
 * @param req Html-Inhalt der transformierten .docx
 * @param res Sendet eine Bestätigung an Frontend
 */
int_router.post('/upload_typo_international', function(req, res) {
  
  var jsonContent = router.transformToJSONInt(req, ordnerPfad, htmlFileName); //JSON-Datensatz der z.B. über jsonContent.content[0].title angesprochen werden 
    ...
    var options = {
      host: 'localhost',
      port: 80,
      path: '/api/rest-api-client/intdokument',
      //path: '/foo',
      method: 'POST',
      headers: {
...
/**
 * @description Internationaler Artikelvorschau, Öffnet die Vorschau in einem neuen Fenster
 * @name get/vorschau_oeffnen_int
 * @param req Html-Inhalt der transformierten .docx
 * @param res Sendet Array an Frontend
 */
int_router.get('/vorschau_oeffnen_int', function(req, res) {

  var resultTrans = router.transformToJSONInt(req, ordnerPfad, htmlFileName); //JSON-Datensatz der z.B. über jsonContent.content[0].title angesprochen werden 
